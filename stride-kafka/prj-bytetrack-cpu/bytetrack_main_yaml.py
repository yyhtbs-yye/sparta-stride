...
from contanos.io.ordered_input_interface import OrderedInputInterface
...
def parse_args():
    ...
    # add_argument(parser, 'in_kafka', 'IN_KAFKA_URL', None)
    add_argument(parser, 'in_kafka', 'IN_KAFKA_URL', 'kafka://localhost:1883,topic=yolox,qos=2,queue_max_len=100')
    # add_argument(parser, 'out_kafka', 'OUT_KAFKA_URL', None)
    add_argument(parser, 'out_kafka', 'OUT_KAFKA_URL', 'kafka://localhost:1883,topic=bytetrack,qos=2,queue_max_len=100')
...
async def main():
    ...
    try:
        ...
        input_interface = OrderedInputInterface(KafkaInput(config=in_kafka_config))
...
async def quick_debug():
    while True:
        ...
        logging.info(f"ORDERED Q: {ordered_q}")
