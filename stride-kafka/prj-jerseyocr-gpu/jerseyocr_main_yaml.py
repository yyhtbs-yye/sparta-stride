...
def parse_args():
    ...
    add_argument(parser, 'in_kafka', 'IN_KAFKA_URL', None) # 'kafka://localhost:1883,topic=bytetrack,qos=2,queue_max_len=100'
    add_argument(parser, 'out_kafka', 'OUT_KAFKA_URL', None) # 'kafka://localhost:1883,topic=jerseyocr,qos=2,queue_max_len=100'
...
async def main():
    ...
    if args.in_kafka:
        ...
        # throw error if not provided
        raise ValueError("IN_KAFKA_URL must be provided as CLI argument.")
