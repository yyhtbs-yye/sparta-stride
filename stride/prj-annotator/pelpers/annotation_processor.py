...
class SkeletonDrawer:
    ...
    def __init__(self, color_manager: ColorManager):
        self.color_manager = color_manager
        self.COCO_SKELETON = np.asarray([
        (15, 13), (13, 11), (16, 14), (14, 12), (11, 12),
        (5, 11), (6, 12), (5, 6), (5, 7), (7, 9),
        (6, 8), (8, 10), (1, 2), (0, 1), (0, 2),
        (1, 3), (2, 4), (3, 5), (4, 6)
    ], dtype=np.int32)
        # Cache of edges valid for a given number of keypoints
        self._edge_cache: Dict[int, np.ndarray] = {}
        # Slightly faster than AA lines
        self._line_type = cv2.LINE_8

    def _edges_for_len(self, n: int) -> np.ndarray:
        """Return cached edges valid for n keypoints as an (m, 2) int32 array."""
        edges = self._edge_cache.get(n)
        if edges is None:
            # Keep only edges whose endpoints exist for length n
            mask = (self.COCO_SKELETON[:, 0] < n) & (self.COCO_SKELETON[:, 1] < n)
            edges = self.COCO_SKELETON[mask]
            self._edge_cache[n] = edges
        return edges
    ...
    def draw(self,
             ...
             thickness: int = 1):
        """Draw skeletons on frame (fast)."""

        polylines = cv2.polylines  # local alias avoids repeated attribute lookups

        for sk in skeletons:
            # Resolve color once
            if color_override and sk.track_id in color_override:
                color = color_override[sk.track_id]
            else:
                color = self.color_manager.get_color(sk.track_id)
            ...
            # Vectorized -> int32 pixel coords (one cast total)
            # NOTE: int rounding is up to you; using np.rint gives symmetrical rounding.
            pts = np.empty((n, 2), dtype=np.int32)
            pts[:, 0] = [int(p.x) for p in kps]
            pts[:, 1] = [int(p.y) for p in kps]

            if draw_bones and n >= 2:
                edges = self._edges_for_len(n)
                if edges.size:
                    # Gather all 2-point segments: shape (m, 2, 2)
                    segs = pts[edges]  # fancy indexing copies, still much faster overall
                    # polylines expects list of arrays of shape (Ni, 1, 2)
                    contours = [s.reshape(2, 1, 2) for s in segs]
                    polylines(frame, contours, isClosed=False,
                              color=tuple(map(int, color)),
                              thickness=thickness, lineType=self._line_type)
...
class AnnotationProcessor:
    ...
    def process_frame(self, frame: np.ndarray, annotation: AnnotationData) -> np.ndarray:
        ...
        # print(f"AnnotationProcessor: Frame {annotation.frame_id} trajectory draw time: {time.time() - now:.3f}s")
        now = time.time()

        # 2. Draw bounding boxes
        color_map = self.bbox_drawer.draw(annotated_frame, scaled_bboxes)
        
        # print(f"AnnotationProcessor: Frame {annotation.frame_id} bbox draw time: {time.time() - now:.3f}s")
        now = time.time()
        ...
        # print(f"AnnotationProcessor: Frame {annotation.frame_id} skeleton draw time: {time.time() - now:.3f}s")
        now = time.time()
        ...
        # print(f"AnnotationProcessor: Frame {annotation.frame_id} jersey draw time: {time.time() - now:.3f}s")
